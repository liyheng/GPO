...
class HumanoidStandup(PipelineEnv):
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    obs, pobs = self._get_obs(rng3, pipeline_state, jp.zeros(self.sys.act_size()))
  ...
  def step(self, rng: jax.Array, state: State, action: jax.Array) -> State:
    ...
    obs, pobs = self._get_obs(rng, pipeline_state, action)
    pobs_ = jp.concatenate([jp.zeros_like(obs), pobs, jp.array([1])])
    obs_ =  jp.concatenate([obs, pobs, jp.array([0])])
  ...
  def _get_obs(
      self, key, pipeline_state: base.State, action: jax.Array
  ) -> jax.Array:
    ...
    pobs = jp.concatenate([
        ...
        com_inertia.ravel(),
        qfrc_actuator,
    ]) + jax.random.normal(key, shape=(155,)) * self.noise_sigma
...
class StatelessHumanoidStandup(HumanoidStandup):
  def __init__(self):
    super().__init__(noise_sigma=0.0)
class NoisyStatelessHumanoidStandupEasy(HumanoidStandup):
  def __init__(self):
    super().__init__(noise_sigma=0.1)
class NoisyStatelessHumanoidStandupMedium(HumanoidStandup):
  def __init__(self):
    super().__init__(noise_sigma=0.2)
class NoisyStatelessHumanoidStandupHard(HumanoidStandup):
  def __init__(self):
