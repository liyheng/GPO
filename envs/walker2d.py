...
class Walker2d(PipelineEnv):
  ...
  def _get_obs(self, key, pipeline_state: base.State) -> jax.Array:
    ...
    pobs = position + jax.random.normal(key, shape=(8,)) * self.noise_sigma
    return obs, pobs
...
class StatelessWalker2d(Walker2d):
  def __init__(self):
    super().__init__(noise_sigma=0.0)
class NoisyStatelessWalker2dEasy(Walker2d):
  def __init__(self):
    super().__init__(noise_sigma=0.1)
class NoisyStatelessWalker2dMedium(Walker2d):
  def __init__(self):
    super().__init__(noise_sigma=0.2)
class NoisyStatelessWalker2dHard(Walker2d):
  def __init__(self):
