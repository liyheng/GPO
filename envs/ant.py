...
class Ant(PipelineEnv):
  ...
  def _get_obs(self, key, pipeline_state: base.State) -> jax.Array:
    ...
    pobs = qpos + jax.random.normal(key, shape=(13,)) * self.noise_sigma
    return obs, pobs
...
class StatelessAnt(Ant):
  def __init__(self):
    super().__init__(noise_sigma=0.0)
class NoisyStatelessAntEasy(Ant):
  def __init__(self):
    super().__init__(noise_sigma=0.1)
class NoisyStatelessAntMedium(Ant):
  def __init__(self):
    super().__init__(noise_sigma=0.2)
class NoisyStatelessAntHard(Ant):
  def __init__(self):
