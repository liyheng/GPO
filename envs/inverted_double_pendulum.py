...
class InvertedDoublePendulum(PipelineEnv):
  ...
  def __init__(self, backend='spring', noise_sigma = 0.0, **kwargs):
    path = (
  ...
  def step(self, rng: jax.Array, state: State, action: jax.Array) -> State:
    ...
    return state.replace(
        pipeline_state=pipeline_state, obs=obs_, pobs=pobs_, reward=reward, done=done
    )
  ...
  def _get_obs(self, key, pipeline_sate: base.State) -> jax.Array:
    ...
    pobs = jp.concatenate([
        ...
        jp.cos(pipeline_sate.q[1:]),
        # qfrc_constraint is not added
    ]) + jax.random.normal(key, shape=(5,)) * self.noise_sigma
    return obs, pobs
...
class StatelessInvertedDoublePendulum(InvertedDoublePendulum):
  def __init__(self):
    super().__init__(noise_sigma=0.0)
class NoisyStatelessInvertedDoublePendulumEasy(InvertedDoublePendulum):
  def __init__(self):
    super().__init__(noise_sigma=0.1)
class NoisyStatelessInvertedDoublePendulumMedium(InvertedDoublePendulum):
  def __init__(self):
    super().__init__(noise_sigma=0.2)
class NoisyStatelessInvertedDoublePendulumHard(InvertedDoublePendulum):
  def __init__(self):
